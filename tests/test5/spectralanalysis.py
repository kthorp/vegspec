...
filepath = './2008_2009_Lesquerella_Biomass_Flower_Silique_CanopyReflectance_PlotAvg.csv'
data = pd.read_csv(filepath,na_values='NA')
print(data)
print(data.shape)
print(data.iloc[0].values[-512:])

wvln = list()
f = open('ger1500wl.txt','r')
lines = f.readlines()
for line in lines:
...
for i in list(range(data.shape[0])):
    spec.append(vs.VegSpec(wvln,np.clip(data.iloc[i].values[-512:],a_min=0.001,a_max=0.9999)))
...
xlab = ['Aboveground biomass (kg ha$^{-1}$)',
        'Flower count',
        'Silique count',
        'Silique weight (kg ha$^{-1}$)']
xval = [1.0,0.1,0.1,0.1]
...
for j, iname in enumerate(spec[0].indices.keys()):
    ...
    string=iname + ','
    for i, x in enumerate(['DryWt','FlowerCount','SiliqueCount','SiliqueDryWt']):
        ...
        if iname in ['NPQI'] and x in ['SiliqueCount']:
            f = open('Fig1d.txt','w')
            f.write(str(slope)+'\n')
