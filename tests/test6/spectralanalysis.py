...
filepath = './2004_Maize_Density6x6_CanopyReflectance_PlotAvg.csv'
data = pd.read_csv(filepath,na_values='NA')
print(data)
print(data.shape)
print(data.iloc[0].values[-120:])

wvln = list()
f = open('rdacsh3-120.txt','r')
lines = f.readlines()
for line in lines:
    wvln.append(float(line.strip().split()[1]))
spec = []

#Load spectra and compute all transformations and indices
for i in list(range(data.shape[0])):
    spec.append(vs.VegSpec(wvln,data.iloc[i].values[-120:]/100.))
    print(i)

#Analyze vegetation indices
fout = open('spectralanalysis.csv','w')

xlab = ['Stand density (plants m$^{-2}$)']
xval = [1.5]
...
for j, iname in enumerate(spec[0].indices.keys()):
    ...
    for i, x in enumerate(['Density']):
        ...
        if iname in ['ESUM1'] and x in ['Density']:
            f = open('Fig1e.txt','w')
            f.write(str(slope)+'\n')
