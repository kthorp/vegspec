...
filepath = './2010_2011_2012_Cotton_Chlorophyll_LeafWater_LeafMass_LAI_CanopyReflectance_PlotAvg.csv'
...
for i in list(range(data.shape[0])):
    spec.append(vs.VegSpec(wvln,np.clip(data.iloc[i].values[-2151:],a_min=0.0001,a_max=0.9999)))
...
xlab = ['Chl $a+b$ ($\mu$g cm$^{-2}$)',
        'Leaf Water Thickness (cm)',
        'Specific Leaf Mass (g cm$^{-2}$)',
        'Leaf area index (m$^2$ m$^{-2}$)']
xval = [28.0,0.012,0.004,2.0]
...
for j, iname in enumerate(spec[0].indices.keys()):
    ...
    string=iname + ','
    for i, x in enumerate(['Cab','H20Thick','SLA3','LAI']):
