...
filepath = './2019_2020_Cotton_Chlorophyll_LeafN_LeafReflectance_PlotAvg.csv'
...
xlab = ['Chl $a$ ($\mu$g cm$^{-2}$)','Chl $b$ ($\mu$g cm$^{-2}$)',
        ...
        'Leaf nitrogen (%)']
xval = [18.0,1.8,18.5,3.0,0.4,3.5,2.0]
...
for j, iname in enumerate(spec[0].indices.keys()):
    ...
    for i, x in enumerate(['ChlAugcm2','ChlBugcm2','ChlABugcm2',
        ...
        if iname in ['DDN'] and x in ['ChlAugcm2']:
            f = open('Fig1a.txt','w')
            f.write(str(slope)+'\n')
        ...
        if iname in ['WLREIPG'] and x in ['ChlABugcm2']:
            f = open('Fig1b.txt','w')
            f.write(str(slope)+'\n')
            f.write(str(intercept)+'\n')
            f.write(str(r*r)+'\n')
